...
analysis of image geolocation. Given a region specified manually or proposed by
WeDetect-Uni, GeoProbe adds
...
on a held-out split. Scoring by clipped mean improvement over the unmodified
model, so that positive values mean reduced error, a three-layer configuration
achieves $+11.94$~km on discovery but $-3.66$~km on holdout
...
$0.9984$, and that displacement is not an artefact of mask size: region
coverage and displacement are negatively related. The results position
GeoProbe as a tool for measuring causal
